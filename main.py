...
class Child(tk.Toplevel):
    def __init__(self):
        ...
        self.head_pose_estimation_model_online = False
        self.var = None
        self.v = None

    def create_child_window(self):
        self.title('Нейронные сети/OpenCV')
        self.geometry('1320x1000+0+0')
        ...
        ch = tk.Label(self, text="1. Распознать возраст/пол", font="times 13")
        ch.place(x=880, y=230)
        button_on = tk.Button(self, text='Включить', command=self.age_gender_begin)
        button_on.place(x=950, y=260)
        che = tk.Label(self, text="2. Распознать эмоцию(ии) человека/людей", font="times 13")
        che.place(x=880, y=300)
        button_begin_1 = tk.Button(self, text='Включить', command=self.emotion)
        button_begin_1.place(x=950, y=330)
        ...
        tInf = tk.Label(self, text="3. Увеличить без потери качества", font="times 13")
        tInf.place(x=880, y=370)
        butto = tk.Button(self, text='Включить', command=self.superres)
        butto.place(x=950, y=400)
        tInf_1 = tk.Label(self, text="4. Распознование лиц/а", font="times 13")
        tInf_1.place(x=880, y=435)
        butto_1 = tk.Button(self, text='Включить', command=self.face_detect)
        butto_1.place(x=950, y=470)
        tInf_2 = tk.Label(self, text="5. Повторное распознование лиц", font="times 13")
        tInf_2.place(x=880, y=500)
        butto_4 = tk.Button(self, text='Включить', command=self.reidentification)
        butto_4.place(x=950, y=530)
        tInf_3 = tk.Label(self, text="6. 5 точек лица", font="times 13")
        tInf_3.place(x=880, y=560)
        butto_6 = tk.Button(self, text='Включить', command=self.landmarks)
        butto_6.place(x=950, y=590)
        tInf_4 = tk.Label(self, text="7. Обнаружить людей/транспортные средства/велосипеды ", font="times 13")
        tInf_4.place(x=880, y=620)
        butto_7 = tk.Button(self, text='Включить', command=self.person_vehicle)
        butto_7.place(x=950, y=650)
        tInf_5 = tk.Label(self, text="8. Контроль положения головы", font="times 13")
        tInf_5.place(x=880, y=700)
        butto_7 = tk.Button(self, text='Включить', command=self.head_pose)
        butto_7.place(x=950, y=730)
        self.combobox = ttk.Combobox(self, values=[u"Change of size", u"Flip", u"Gaussian blur",
                                                   u"Black and white filter","Kenny border detection"])
        ...
        butt_6.place(x=900, y=0)
        butt_7 = tk.Label(self, text='Информация о моделях', font="times 13")
        butt_7.place(x=940,y=8)
        btn_cancel = ttk.Button(self, text='Закрыть', command=self.destroy)
        btn_cancel.place(x=1200, y=750)
        ...
        self.parser.add_argument('-mHP', dest='head_pose_estimation_model', default='head-pose-estimation-adas-0001',
                            help='Path to the model_8')
    ...
    def getValue(self, video_flag, webcam_flag, image_flag):
        if video_flag == True:
    ...
    def head_pose_estimation_model_init(self):
        net_HP = self.ie.read_network(self.args.head_pose_estimation_model + '.xml',
                                      self.args.head_pose_estimation_model + '.bin')
        exec_net_HP = self.ie.load_network(net_HP, 'CPU')
    ...
    # face_detection_model
    def face_detection(self, frame, net_FD, exec_net_FD, net_AGR, exec_net_AGR, net_ER,
                       exec_net_ER,net_LR, exec_net_LR, net_HP, exec_net_HP):
        ...
        for out in outs:
            ...
            if conf > 0.8:
                ...
                if broken_ROI_face == False:
                    ...
                    if self.head_pose_estimation_model_online == True:
                        frame = self.head_pose_estimation(frame, ROI_face, net_HP, exec_net_HP, coord)
    ...
    def head_pose_estimation(self, frame, ROI_face, net_HP, exec_net_HP, coord):
        size = 100
    ...
    def face_detect(self):
        ...
        net_HP, exec_net_HP = self.head_pose_estimation_model_init()

        self.face_detection_model_online = True


        if image_flag == True:
            picture = self.getValue(video_flag, webcam_flag, image_flag)
            if self.face_detection_model_online == True:
            ...
            cap = self.getValue(video_flag, webcam_flag, image_flag)
            while (cap.isOpened()):
                # Reading the frame
                ret, frame = cap.read()
                ...
                if self.face_detection_model_online == True:
                    frame, ROI_face = self.face_detection(frame, net_FD, exec_net_FD, net_AGR, exec_net_AGR, net_ER,
                                                          exec_net_ER, net_LR,
                                                          exec_net_LR, net_HP, exec_net_HP)
    ...
    def head_pose(self):
        ...
        self.head_pose_estimation_model_online = True
    ...
    def opencv_run(self):
        ...
        if res == "Change of size":
            ...
            if image_flag == True:
                ...
                messagebox.showinfo("ОШИБКА", "Загрузите фото/видео или включите веб камеру!!!")
        elif res == "Kenny border detection":
            if image_flag == True:
                gr = gray(im)
                edges = cv.Canny(gr, 100, 100)
                self.location(edges)
            elif video_flag == True or webcam_flag == True:
                while (cap.isOpened()):
                    ret, img = cap.read()
...
class Child_1(tk.Toplevel):
    ...
    def inf_about_models(self):
        ...
        self.geometry('1100x530+200+200')
        ...
        TEXT_2 = tk.Label(self,
                          text="Подход, основанный на внимании к сверхразрешению одного изображения, "
                               "но с уменьшенным числом каналов и изменениями в сетевой архитектуре.",
                          font="times 13")
        ...
        TEXT_5 = tk.Label(self,
                          text="Это модель повторной идентификации человека. "
                               "Найденные объекты регестрируются в базе данных.",
                          font="times 13")
        TEXT_5.place(x=0, y=416)
        TEXT_6 = tk.Label(self, text="8. head-pose-estimation-adas-0001:", font="times 13")
        TEXT_6.place(x=0, y=445)
        TEXT_7 = tk.Label(self, text="Сообщает об оцененной позе головы в углах.", font="times 13")
        TEXT_7.place(x=0, y=475)
        btn_cancel = ttk.Button(self, text='Закрыть', command=self.destroy)
        btn_cancel.place(x=980, y=480)
        self.grab_set()
        self.focus_set()
        self.wait_window()
